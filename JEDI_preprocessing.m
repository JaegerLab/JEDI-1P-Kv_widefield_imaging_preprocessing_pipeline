...
green_pass_fs = parameters.green_pass_fs; % for lowpass filter fro JEDI-1P-Kv
red_pass_heartbeat = parameters.red_pass_heartbeat;% band pass filter to extract heartbeat at its harmonics (Step 1)
red_pass_motion = parameters.red_pass_motion; %band pass filter to extract potential motion artifact (Step 2)
red_pass_slowHemo= parameters.red_pass_slowHemo;  % lowpass filter to extract slow hemodynamics (Step 3)
...
% this should only happen to pixel around the edge with no signal from the
% brain
if baseLine_r ~= 0
...
function [regressed_B] = detrendingFunction(trace_A, trace_B, IMG_sampleRate)
    trace2regress = trace_B; % trace to be regressed: raw JEDI-1P-Kv channel
    rg = trace_A;  % trace to regress from: fitted photobleaching decay
